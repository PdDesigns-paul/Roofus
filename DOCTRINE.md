...
| **Streets** | Age-band **park-once loops** named from Census streets (or a small CDP), grouped by county then township. Working pinned. Search. Rural counties still get a row. Maps parks on the loop. One weather sentence on the Working loop if a kept storm matches. **Use today** adds that loop to Today’s plan. Last 48 hours and season log are **not** on this page. Not a developer-subdivision dump. Not one card per block group. |
| **Storms** | Not a tab. Satellite map of storms they **Keep**. General pointers, not a house. Pending last-48h as a list with Keep / Toss. Pins only move after Keep. Empty: age first. Open from Menu, Presets, or the morning nag. Today stays the log. |
