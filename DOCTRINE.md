...
- Account is a key on You, not a `/login` Place. Chip reads **This phone** when signed out. Switch book is a chip on You when another owner is already on this phone — typed confirm if Today has counts or pins, same ritual as Restore. No Stripe sheet.
...
- The phone is the live log. A file or optional Notion is a copy so a dead phone is not a dead year.
...
| **Reference** | Reference page lists the InterNACHI index. Coach appendix is the named cards. Company knowledge is the website crawl and packet files they photographed or uploaded. Coach may open a named card URL or a company page they saved, then coach from original notes. No text, no “I read your flyer.” Name the title. Do not paste licensed article bodies. |
...
| **Settings** | Index of pages: You, Territory, Hours (policy windows + this week's clock, conversion, and a picture under the numbers — first-party, not hidden from the canvasser; Last 30 is a chip; those numbers stay after old days prune), Mindset, Reminders, Backup. Each row is a Go with a chevron. Plan is its own tab, not a Settings row. Show the tour and Load a sample day are outlined pills under the list. Name, counties, state, hours, company, website (crawled into Reference), packet files they photographed or uploaded (saved text only), warranty, mindset worksheets, last copy on You, Backup = Copy this phone (file or optional Notion). Restore onto a full day or pins is a typed confirm. Memory FAQs. He can write these when they clearly set them. Labor segments and the Hours rollup ride in the copy. Trail points stay off Notion. |
...
Clock is first-party. Pause is a chip. Work windows and minutes, not one fake start. Tiles and pin status stamp a time — Hours graphs those times. Empty clocks are gaps, not 0h bars. Trail is opt-in, foreground only, a quiet line under pins — not a pin, not a live avatar. Coach gets a sample count or the sentence “Trail is off.” Never lat/lng. Quote an hour split only when stamps exist. Fat days prune at 60; a compact rollup of minutes and counts keeps Last 30 honest. Office copy carries segments + rollup. Not thousands of trail points.
