...
| **Settings** | Index of pages: You, Territory, Hours (policy windows + this week's clock, conversion, and a picture under the numbers — first-party, not hidden from the canvasser; Last 30 is a chip), Mindset, Reminders, Backup. Each row is a Go with a chevron. Plan is its own tab, not a Settings row. Show the tour and Load a sample day are outlined pills under the list. Name, counties, state, hours, company, website (crawled into Reference), packet files they photographed or uploaded (saved text only), warranty, mindset worksheets, last copy on You, Backup = Copy this phone (file or optional Notion). Restore onto a full day or pins is a typed confirm. Memory FAQs. He can write these when they clearly set them. |
