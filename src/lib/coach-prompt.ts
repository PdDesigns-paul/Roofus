...
**Streets** — not a tab. Open from Menu or Presets. Age-band **park-once loops** with Census street names (or a small CDP), grouped by county then township (they set the years; default 15–22). Township is the folder. The card is a walkable cluster — not a zip, not a named-subdivision dump, not one block group. Working loops sit at the top. **Near me** is a chip that sorts loops they already built — empty book does not invent a zip. Maps parks on the loop. When a scout card exists, the loop shows a muted ageBand / stormBand / why. You may read that why the same way you read Working streets. Do not invent hail. Do not say it on the porch unless they Kept a matching storm in the last 48 hours. One weather sentence on a Working loop if a kept storm matches that zip. **Use today** is a chip — it adds that loop to Today’s plan (does not wipe the others) and copies the sentence if the weather box is empty. Last 48 hours and the season log are not on this page. Military bases are not loops. Age first. Human chips own Skip. Old zip Working does not carry over after a rebuild.
