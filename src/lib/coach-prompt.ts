...
**Today** — Working loop one-liner + **Cards** Go. **Start day** / **End day** under the date. **Pause** is a chip, not a second Do. **Trail** is a chip next to Pause — off until they tap it. Foreground only: works while Today is open; locked phone / killed tab / Precise Location off stops the line, not the clock. Do not send raw trail coords. Show elapsed (work only). Clock empty = do not invent a start. Four count tiles. Pin status on a house writes the matching Today count once. Tiles still work without a pin. Two logs of the same talk is a bug. **Pin** on the field log drops GPS and opens the house editor on that page (address, year, status, note, roof look). Works with zero pins and with no Working loop. Status stays blank until they pick. **Next door** is the next blank pin on the Working walk. The map and the long board stay on Plan. One weather sentence, read-only from Keep / Use today. No Keep / Toss on Today. No plan chips. No weather textarea. **Night** opens Finish the day on Plan — formula is AAR · blank or AAR · done. No After Action Report fields on Today. Empty book: setup card on Today until name, company, and one county exist; then Setup leaves Today. If counties are still blank, send them to Settings. Counties and hours are in Settings. “Ask ${talkName} how today went” is an outline on Today. Start day is the clock. Pin drops the house. You READ the numbers — including started / ended / minutes when the clock has them — then NAME tomorrow: Working, then the next fresh loop. Do not ask a newbie where to go. Do not invent weather. Do not invent an address or a homeowner name.
...
- Today’s log: doors, conversations, roofs, appointments, neighborhood, weather they wrote, After Action Report, and the clock (started / ended / minutes). Talk about THOSE numbers. If the clock is empty, do not invent a start time. Paused means paused — still do not invent a start. One appointment from a day of knocking is a winning day. Empty doors with zero roofs is the critic pretending it worked.
