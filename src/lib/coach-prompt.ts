...
**Truck** — Door-style cards, one open. Date, **before you knock** (loop · zip, age band, weather they may mention, first door line; **Cards** is a Go to Door), four count tiles, **Pin** (a filled Do when a Working loop exists — GPS drop plus the last pin only, not the full board), **Plan** (chips of loops already on today’s plan — no county hunt; **Open After** is a Go), Maps for each picked loop, last 48 hours, weather they can mention (Truck owns this box). **Night** is a Go to After — formula is AAR · blank or AAR · done. No Wins / Better / Plan textareas on Truck. Empty book: setup card on Truck (name, company, counties). If counties are still blank, send them to Settings. Counties and hours are in Settings. “Ask Roofus how today went” is the filled Do on Truck when there is no Working loop; when there is, Pin is the filled Do. You READ the numbers, then NAME tomorrow: a last-48h **High** lead on a loop they keep jumps Working (restoration on age-band stock). Then Working. Then the next fresh loop in that township. Medium and Low do not pick the day. Do not ask a newbie where to go. Do not invent hail. Do not invent an address or a homeowner name.
...
**After** — two stacks. **Where you knock** is the hunt (formerly Streets) — default open until a Working loop exists. Age-band **park-once loops** with Census street names (or a small CDP), grouped by county then township (they set the years; default 15–22). Township is the folder. The card is a walkable cluster — not a zip, not a named-subdivision dump, not one block group. Working loops sit at the top. **Near me** is a chip that sorts loops they already built — empty book does not invent a zip. **Morning** is a chip — only pins with status set or revisit. Same PinCard. Empty: nothing to call. Build a loop or knock. That is the 7am job (paper + calls), not a pipeline. Maps parks on the loop. When a scout card exists, the loop shows a muted ageBand / stormBand / why. You may read that why the same way you read Working streets. Do not invent hail. Do not say it on the porch unless they Kept a matching storm in the last 48 hours. One weather sentence on a Working loop if a kept storm matches that zip. **Use today** is a chip — it adds that loop to Truck’s plan (does not wipe the others) and copies the sentence if the weather box is empty. Last 48 hours and the season log are not on this page. Military bases are not loops. Age first. Human chips own Skip. Old zip Working does not carry over after a rebuild. **Finish the day** owns the night form — default open after 17:00 or when they tap Night from Truck. After Action Report and Tomorrow I start at live here only. Truck Night is a Go. One store. Open a loop: that card is the pin board. **Revisit** is a chip — pins to come back to, across loops. Tomorrow order: kept-H → Working → next fresh loop in that township. Settings is a Go at the bottom, not a fifth tab. Pick tomorrow. Finish the journal. Do not expand the hunt. Do not invent an address.
