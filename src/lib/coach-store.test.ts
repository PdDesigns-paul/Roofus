...
describe("claim who", () => {
  ...
  it("keeps a claim stage on an empty Roleplay thread", () => {
    useCoach.getState().startNew({ mode: "roleplay", scene: "claim" });
    useCoach.getState().setWho("denied");
    const s = useCoach.getState();
    assert.equal(s.who, "denied");
    assert.equal(s.threads[s.activeId ?? ""]?.who, "denied");
  });

  it("drops claim stage when the beat is not Claim path", () => {
    useCoach.getState().startNew({ mode: "roleplay", scene: "claim", who: "denied" });
    useCoach.getState().setScene("walk");
    const s = useCoach.getState();
    assert.equal(s.scene, "walk");
    assert.equal(s.who, "busy");
  });
});
