        ...
        <ul className="mb-4 flex flex-col gap-1">
          {due.map((r) => (
            <li key={r.id} className="flex min-h-11 items-center justify-between gap-2 text-sm">
              <Link
                to={r.to}
                className="min-w-0 text-muted underline-offset-4 hover:text-fg hover:underline"
              >
                {r.label}
              </Link>
              <button
            ...
            {SETUP_ROWS.map((row) => {
              ...
              return (
                <li key={row.id} className="flex items-stretch gap-2 border-b border-border last:border-0">
                  <Link to={row.path} className="flex min-h-14 min-w-0 flex-1 items-center gap-3 py-3">
                    <span
