...
| Surface | What it is |
| --- | --- |
| **Truck** | Field log. Working loop one-liner + Cards Go. **Pin** drops GPS and opens the house editor on this page (address, year, status, note, roof look). Works with no Working loop. **Next door** is the next blank pin on that walk. Four counts. One weather sentence, read-only from Keep / Use today. **Night** is a Go to Prep#finish (AAR · blank or AAR · done). Empty-book setup until name + company + one county exist. Ask Roofus how today went is an outline. Pin is the filled Do. The map and the long board stay on Prep. |
