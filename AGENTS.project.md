...
- Pins are the hunt. Drop from Today (GPS) — that opens the house editor on Today (address, year, status, note, roof look) — or the Plan map (Desk). Pin status writes the matching Today count once. Tiles still work without a pin. Distance makes the loop. Sidewalk house log, not a CRM, not a pipeline. No takeoff, no scrape. Street View / Zillow / Redfin are outbound links.
