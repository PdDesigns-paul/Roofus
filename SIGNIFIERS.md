...
- Settings index: rows get chevrons. Plan is a tab, not a Settings row. “Show the tour” and “Load a sample day” stay secondary outlined pills, below the list, not dressed as the page’s primary. You: owner chip. Switch book is a chip when another book is already on this phone — not a header UserButton. Office is a desk URL (`/settings/office`), not a Place and not a Settings row. Choose a mark is the Do. Pack file is a chip. Pack `roofus` metal is frozen.
